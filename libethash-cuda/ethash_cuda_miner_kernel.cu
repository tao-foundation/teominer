...
ethash_calculate_dag_item(uint32_t start)
{
	...
	for (uint32_t i = 0; i != ETHASH_DATASET_PARENTS; ++i) {
		...
		for (uint32_t t = 0; t < 4; t++) {
			...
			for (int w = 0; w < 4; w++) {
				...
				uint4 s4 = make_uint4(__shfls(0xFFFFFFFF,p4.x, w, 4), __shfls(0xFFFFFFFF,p4.y, w, 4), __shfls(0xFFFFFFFF,p4.z, w, 4), __shfls(0xFFFFFFFF,p4.w, w, 4));
				if (t == thread_id) {
	...
	for (uint32_t t = 0; t < 4; t++) {
		...
		for (uint32_t w = 0; w < 4; w++) {
			s[w] = make_uint4(__shfls(0xFFFFFFFF,dag_node.uint4s[w].x, t, 4), __shfls(0xFFFFFFFF,dag_node.uint4s[w].y, t, 4), __shfls(0xFFFFFFFF,dag_node.uint4s[w].z, t, 4), __shfls(0xFFFFFFFF,dag_node.uint4s[w].w, t, 4));
		}
